"""
Universal Blog Writing Agent with Dynamic Research & Multimodal Image Generation
================================================================================

Architecture Overview:
----------------------
This module implements an Orchestrator-Worker (Map-Reduce) agent with LangGraph.
It can research, plan, write, and visually illustrate blogs across any genre
(technical, personal narrative, thought leadership, guides, case studies, opinions, etc.).

Workflow Stages:
1. router_node:
   - Evaluates the topic, tone, and audience.
...
3. orchestrator_node:
   - Synthesizes the topic, evidence, audience, and genre into a structured outline (Plan).
   - Breaks down the post into 5-9 concrete sections (Task objects) with goals and bullet points.
...
# ============================================================================
# 1. Pydantic Schemas (Data Contracts & LLM Structured Outputs)
# ============================================================================
# Note: Pydantic BaseModels are used for external/LLM data boundaries to provide
# strict schema validation, field descriptions, and automatic type coercion.

class Task(BaseModel):
    """Defines a single section/chapter within the planned blog post."""
    id: int = Field(..., description="Unique 1-based sequential index for this section.")
    title: str = Field(..., description="Engaging section title.")
    section_type: str = Field(
        default="body",
        description="Role of this section: intro, body, narrative_arc, case_example, practical_steps, reflection, conclusion, etc.",
    )
    goal: str = Field(
        ...,
        description="One sentence describing what the reader should learn, feel, or take away after reading this section.",
    )
    bullets: List[str] = Field(
        ...,
        min_length=3,
        max_length=6,
        description="3–6 concrete, non-overlapping key points, arguments, or narrative beats to cover.",
    )
    target_words: int = Field(..., description="Target word count for this section (120–550 words).")
    tags: List[str] = Field(default_factory=list, description="Tags or keywords relevant to this section.")
...
class Plan(BaseModel):
    """The master blog blueprint generated by the orchestrator."""
    blog_title: str = Field(..., description="Captivating, click-worthy, and authentic blog title.")
    audience: str = Field(..., description="Target audience (e.g., curious generalists, seasoned developers, product managers).")
    tone: str = Field(..., description="Voice and style (e.g., conversational, witty, analytical, intimate, journalistic).")
    blog_kind: Literal[
        "explainer",          # Deep-dive concept or mechanism
        "tutorial",           # Practical how-to guide
        "opinion_editorial",  # Strong viewpoint, thought-provoking stance
        "story_narrative",    # Personal story, founder journey, anecdote-driven
        "case_study",         # Real-world problem, decision breakdown, results
        "thought_leadership", # Industry trend analysis, forward-looking insights
        "review_critique",    # Product, book, tool, or cultural evaluation
        "guide",              # Comprehensive manual or playbook
        "news_roundup",       # Event synthesis, highlights, implications
        "comparison",         # Side-by-side trade-offs and benchmarks
    ] = Field(default="explainer", description="The genre and format of the blog post.")
    constraints: List[str] = Field(default_factory=list, description="Specific stylistic or structural constraints.")
    tasks: List[Task] = Field(..., description="Ordered list of sections that make up the blog.")
...
ROUTER_SYSTEM = """You are an expert editorial strategist and research director.
Your role is to analyze the user's blog topic, requested tone, and audience, and determine
whether live web research is needed BEFORE writing the outline.
...
ORCH_SYSTEM = """You are a master editor-in-chief and publishing strategist.
Your mission is to architect a compelling, logically structured outline for a blog post
that resonates deeply with readers and fits the requested topic and genre.

Broad Genre Adaptability:
- This blog can be of ANY genre: technical deep-dive, personal story/narrative, thought leadership,
...
- Adapt the structure, tone, and goals to the specific subject:
  * For personal/narrative posts: Focus on storytelling beats, tension, pivotal choices, and hard-won reflections.
  ...
  * For technical posts: Include architecture intuitions, code/design trade-offs, and edge cases.
...
def orchestrator_node(state: State) -> dict:
    """Produces the structured blog outline (Plan) using available topic and research evidence."""
...
Write ONE complete, polished section of a blog post in Markdown.
...
- Start immediately with a '## <Section Title>' heading. Do NOT output a top-level '# Blog Title' H1.
...
DECIDE_IMAGES_SYSTEM = """You are an expert creative director and editorial illustrator.
Review the complete blog draft and decide where visual aids, diagrams, or illustrations
will materially elevate the reader's understanding and aesthetic engagement.

Visual Strategy Guidelines:
- Maximum 3 images per blog post. Only propose images where they add genuine value.
- Style adaptation based on blog genre:
  ...
  * Lifestyle / Culture: Stylized editorial photography or contextual visual compositions.
